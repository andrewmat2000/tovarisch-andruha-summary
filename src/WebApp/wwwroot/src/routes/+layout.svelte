<script lang="ts">
  ...
  let links: (LinkType & { authorizationRequire: boolean })[] = [];

  onMount(async () => {
    await waitLocale();
    links = [
      {
        authorizationRequire: false,
        label: $_("left-panel-link-main"),
        path: "/",
      },
      {
        authorizationRequire: true,
        label: $_("left-panel-link-admin"),
        path: "/admin",
      },
    ];
  });
      ...
      <body class="mobile">
        {#if showMenu}
          <button id="menu" on:click={() => (showMenu = false)}>
            {#each links as link}
              {#if !link.authorizationRequire || $uiStore.isAuthorized}
                <Link {link} callback={() => (showMenu = false)} />
              {/if}
            {/each}
          </button>
        {/if}
        <header>
          <button id="back-button" class={`arrow`} style={`opacity: ${$uiStore.url.length > 1 ? "1" : "0"};`} on:click={() => uiStore.gotoBack()}>
            <div></div>
          </button>
          <span></span>
      ...
      <body class="desktop">
        ...
        <div>
          ...
          <footer>
            ...
            <a href="https://github.com/andrewmat2000/tovarisch-andruha-summary" target="_blank">
